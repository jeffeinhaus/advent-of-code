...
const hasRequiredField = (field: string, passport: string): boolean => passport.includes(field);
...
export const hasValidDateInRange = (
  field: string,
  passport: string,
  minDate: number,
  maxDate: number,
...
export const hasValidHeight = (passport: string): boolean => {
  ...
  if (fieldToValidate) {
    ...
    return (
      (value.endsWith('cm') &&
        parseInt(value.substring(0, 3)) >= 150 &&
        parseInt(value.substring(0, 3)) <= 193) ||
      (value.endsWith('in') &&
        parseInt(value.substring(0, 2)) >= 59 &&
        parseInt(value.substring(0, 2)) <= 76)
    );
  }
  return false;
};
