export const maxSeatId = (boardingPasses: string[]): number => {
  return Math.max(...boardingPasses.map((boardingPass) => computeSeatId(boardingPass)));
}

export const findMySeat = (boardingPasses: string[]): number => {
  const seats = boardingPasses.map((boardingPass) => computeSeatId(boardingPass))
    .sort((a, b) => (a - b));
...
const binarySearch = (partitions: string, minRegion: string, maxRegion: string, minBound: number, maxBound: number): number => {
