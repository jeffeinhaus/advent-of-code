export const sumMemoryValues = (instructions: string[]): number => {
  ...
  instructions.forEach((instruction) => {
    if (instruction.startsWith('mask')) {
      ...
      const memoryAddress = Number(instruction.substring(instruction.indexOf('[') + 1, instruction.indexOf(']')));
...
export const sumFloatingMemoryValues = (instructions: string[]): number => {
  ...
  instructions.forEach((instruction) => {
    if (instruction.startsWith('mask')) {
      ...
      const memoryAddress = (Number(instruction.substring(instruction.indexOf('[') + 1, instruction.indexOf(']'))) >>> 0).toString(2);
      const zeroPaddedAddress = Array(mask.length - memoryAddress.length + 1).join('0') + memoryAddress;
