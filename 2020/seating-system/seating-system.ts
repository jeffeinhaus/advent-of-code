export const findOccupiedSeatsAtEquilibrium = (seatingChart: string[][], strict: boolean, tolerance: number): number => {
  let updatedSeatingChart = seatingChart.map((row) => row.slice());
  let seatsChanged = 0;
  do {
    let previousSeatingChart = [...updatedSeatingChart];
    const adjacentOccupiedSeats: number[][] = getAdjacentOccupiedSeats(previousSeatingChart, strict);
    
    seatsChanged = 0;
    [updatedSeatingChart, seatsChanged] = updateSeatingChart(previousSeatingChart, adjacentOccupiedSeats, tolerance);
  } while (seatsChanged != 0);

  let occupiedSeats = getOccupiedSeats(updatedSeatingChart);
  return occupiedSeats;
};

const updateSeatingChart = (seatingChart: string[][], adjacentOccupiedSeats: number[][], tolerance: number): [string[][], number] => {
  let seatsChanged = 0;
  let updatedSeatingChart = seatingChart.map((row) => row.slice());
  seatingChart.forEach((row, rowIndex) => {
    row.forEach((column, columnIndex) => {
      if (seatingChart[rowIndex][columnIndex] === 'L' && adjacentOccupiedSeats[rowIndex][columnIndex] === 0) {
        updatedSeatingChart[rowIndex][columnIndex] = '#';
        seatsChanged++;
      } else if (seatingChart[rowIndex][columnIndex] === '#' && adjacentOccupiedSeats[rowIndex][columnIndex] >= tolerance) {
...
export const findAdjacentOccupiedSeatsStrict = (seatingChart: string[][], row: number, column: number): number => {
  ...
  for (let i = row + 1, j = column + 1; i < seatingChart.length && j < seatingChart[row].length; i++, j++) {
    if(seatingChart[i][j] != '.') {
...
const findAdjacentOccupiedSeats = (seatingChart: string[][], row: number, column: number): number => {
  let numOccupiedSeats = 0;
  ...
  if (row - 1 >= 0 && column + 1 < seatingChart[row].length &&  seatingChart[row - 1][column + 1] === '#') {
    numOccupiedSeats++;
  }
  ...
  if (row + 1 < seatingChart.length && column - 1 >= 0 && seatingChart[row + 1][column - 1] === '#') {
    numOccupiedSeats++;
  }
  if (row + 1 < seatingChart.length && seatingChart[row + 1][column] === '#') {
    numOccupiedSeats++;
  }
  if (row + 1 < seatingChart.length && column + 1 < seatingChart[row].length && seatingChart[row + 1][column + 1] === '#') {
    numOccupiedSeats++;
  }
  return numOccupiedSeats;
};